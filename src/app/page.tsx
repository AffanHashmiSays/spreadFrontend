...
async function fetchHomePageData(limit: number) {
  try {
    const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    // Detect mobile from user-agent
    const [postsResponse, categoriesResponse, tagsResponse] = await Promise.all([
      fetch(`${apiBaseUrl}/posts?page=1&limit=${limit}`, {
        next: { 
...
export default async function HomePage() {
  ...
  const { posts, categories, tags, pagination } = await fetchHomePageData(limit);
  
  return <ArticlesSSR initialPosts={posts} initialCategories={categories} initialTags={tags} initialPagination={pagination} />;
}