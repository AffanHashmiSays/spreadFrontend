...
export function PostsList() {
  ...
  // Load posts and categories with pagination
  const loadData = async (page = 1, limit = itemsPerPage) => {
    const token = localStorage.getItem('crm_token');
    setLoading(true);
    setError(null);
    try {
      const [postsResponse, categoriesData] = await Promise.all([
        api.get(`/posts?page=${page}&limit=${limit}`, token || undefined),
        api.get('/categories', token || undefined),
      ]);
      
      // Handle posts response based on API format
      let postsData = [];
      let total = 0;
      let totalPages = 1;
      
      // Check if response has pagination data
      if (postsResponse && postsResponse.pagination) {
        // New format with pagination object
        const { currentPage, totalPages: pages, totalPosts } = postsResponse.pagination;
        postsData = Array.isArray(postsResponse.data) ? postsResponse.data : [];
        total = totalPosts || 0;
        totalPages = pages || 1;
        
        // Update current page from the API response
        setCurrentPage(currentPage);
      } else if (Array.isArray(postsResponse)) {
        // Fallback: if API returns array directly
        postsData = postsResponse;
        total = postsResponse.length;
        totalPages = 1;
      } else if (postsResponse && typeof postsResponse === 'object') {
        // Alternative format with direct properties
        postsData = Array.isArray(postsResponse.data) ? postsResponse.data : [];
        total = postsResponse.total || postsResponse.totalPosts || 0;
        totalPages = postsResponse.pages || postsResponse.totalPages || 1;
      }
      
      const normalizedCategories = Array.isArray(categoriesData) ? categoriesData : categoriesData?.data || [];
  ...
  // Use posts directly from state (already paginated by the server)
  const currentPosts = posts;
  const { total = 0, page: currentPageFromApi = currentPage, totalPages = 1 } = pagination || {};
  const startIndex = total > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0;
  const endIndex = total > 0 ? Math.min(startIndex + itemsPerPage - 1, total) : 0;

  // Reset to first page when search changes or image filter changes
  useEffect(() => {
    setCurrentPage(1);
    // Reload data with the first page when search or filter changes
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center flex-1">
          ...
          <TooltipProvider>
            <div className="flex items-center gap-2">
              <span className="hidden md:inline text-sm text-gray-700">Filtres d'images:</span>
      ...
      {/* Posts Table */}
      <div className="border rounded-lg w-[95%] lg:w-[100%]">
        ...
        <div className="md:hidden space-y-2 px-2 max-w-2xl mx-auto w-full">
          {currentPosts.length === 0 ? (
            ...
            currentPosts.map((post) => (
              <div
                key={post._id || post.id}
                className={cn(
                  "border rounded-lg p-4 bg-white shadow-sm w-full",
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem asChild>
                        <Link href={`/dashboard/posts/${post._id || post.id}`} className="w-full">
                          <Edit className="mr-2 h-4 w-4" />
                          Modifier
                        </Link>
                      </DropdownMenuItem>
                ...
                <div className="space-y-2 mt-2">
                  <div className="flex flex-wrap gap-1">
                    {getCategoryNames(post.categoryIds).map((categoryName, index) => (
                      <Badge key={index} variant="outline" className="text-[11px] h-5">
                        {categoryName}
                      </Badge>
                    ))}
                  </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm text-gray-700">
            Affichage de {startIndex} à {endIndex} sur {total} articles
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center space-x-2">
          <Button
            ...
            className={`min-w-[100px] ${currentPage === 1 || totalPages <= 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <ChevronLeft className="h-4 w-4 mr-1" />
            Précédent
          </Button>
          
          {/* Page numbers - only show if more than one page */}
          {totalPages > 1 && (
          ...
          <Button
            ...
            className={`min-w-[100px] ${currentPage === totalPages || totalPages <= 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            Suivant
