...
export function PostForm({ isNewPost = false }: PostFormProps) {
  const params = useParams<{ id: string }>();
  const id = params?.id;
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  
  // If isNewPost is true, we're creating a new post
  const isCreating = isNewPost || !id || id === 'new';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If editing, set the initially selected categories
        if (id && id !== 'new') {
          const post = await api.get(`/posts/${id}`, token || undefined);
          if (post.categoryIds && post.categoryIds.length > 0) {
            // Extract just the IDs from the category objects
            const categoryIds = post.categoryIds.map((cat: any) => cat.id || cat._id);
            setSelectedCategories(categoryIds);
            
            // Set the parent category if it exists
            const parentCategory = post.categoryIds.find((cat: any) => !cat.parentId);
            if (parentCategory) {
              setSelectedParentCategory(parentCategory.id || parentCategory._id);
            }
            
            // Set the subcategory if it exists
            const subCategory = post.categoryIds.find((cat: any) => cat.parentId);
            if (subCategory) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Convert metaKeywords to a comma-separated string if it's an array
      const formatMetaKeywords = (keywords: string | string[]): string => {
      ...
      const postData = {
        ...
        metaKeywords: metaKeywords ? formatMetaKeywords(metaKeywords) : undefined,
      ...
      let response;
      // If we're creating a new post or editing an existing one
      if (!id || id === 'new') {
  ...
  // Get selected parent categories and their subcategories
  const selectedParentCategories = (categories || []).filter(
    cat => cat && cat._id && selectedCategories.includes(cat._id) && !cat.parentId
  );
  ...
  const toggleCategory = (categoryId: string, isParent: boolean) => {
    setSelectedCategories(prev => {
      // If it's a parent category, we need to handle its subcategories
      if (isParent) {
        const isSelected = prev.includes(categoryId);
        if (isSelected) {
          // If unselecting a parent, also unselect all its subcategories
          const subCats = categories
            .filter(cat => cat.parentId === categoryId)
            .map(cat => cat._id);
          return prev.filter(id => id !== categoryId && !subCats.includes(id));
        } else {
          // If selecting a parent, just add it (don't auto-select subcategories)
          return [...prev, categoryId];
        }
      } else {
        // For subcategories, just toggle the selection
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <Label className="block mb-2">Catégories principales</Label>
                ...
                return (
                  <div key={`parent-${categoryId}`} className="flex items-center space-x-2">
                    <Checkbox
                      id={`cat-${categoryId}`}
                      checked={isChecked}
                      onCheckedChange={() => toggleCategory(categoryId, true)}
                    />
                    <Label htmlFor={`cat-${categoryId}`} className="cursor-pointer">
                      {category.name}
                    </Label>
                  </div>
                );
            ...
            {selectedParentCategories?.length > 0 && (
              <div>
                <Label className="block mb-2">Sous-catégories</Label>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                  {subCategories
                    .filter(sub => selectedParentCategories.some(parent => parent?._id === sub?.parentId))
                    .map((sub, index) => {
                      ...
                      return (
                        <div key={`sub-${subId}`} className="flex items-center space-x-2 ml-4">
                          <Checkbox
                            id={`sub-${subId}`}
                            checked={isChecked}
                            onCheckedChange={() => subId && toggleCategory(subId, false)}
                          />
                          <Label htmlFor={`sub-${subId}`} className="cursor-pointer">
                            {sub?.name || 'Sans nom'}
                          </Label>
                        </div>
                      );
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {tags.map((tag) => {
                // Ensure we have a valid ID for the key
                const tagId = tag.id || tag._id || `temp-tag-${Math.random().toString(36).substr(2, 9)}`;
                return (
                  <div key={`tag-${tagId}`} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`tag-${tagId}`} className="cursor-pointer">
                      {tag.name}
                    </Label>
                  </div>
                );
